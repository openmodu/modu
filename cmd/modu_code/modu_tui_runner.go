...
func runModuTUI(ctx context.Context, session *coding_agent.CodingSession, model *types.Model, noApprove bool, opts RunOptions) (err error) {
	...
	refreshWorkflowPanelForRun := func(runID string) bool {
		workflowPanelMu.Lock()
		if !workflowPanelActive {
			workflowPanelMu.Unlock()
			return false
		...
		workflowPanelMu.Lock()
		shouldSend := false
		if workflowPanelActive && workflowPanelRef == ref && ref.MatchesRun(runID) {
			workflowPanelFingerprint = fingerprint
			shouldSend = true
		}
		workflowPanelMu.Unlock()
	...
	unsubAgent := session.Subscribe(func(ev types.Event) {
		...
		if panel, status, ok := moduTUIWorkflowPanelFromToolEvent(session, ev); ok {
			...
			if panel.ID != "" {
				...
				if runID == "" || !refreshWorkflowPanelForRun(runID) {
					send(modutui.SetPanelMsg{Panel: panel})
				}
			}
		}
...
func moduTUIWorkflowCockpitPanelFromStatesWithText(states map[string]any, text string) modutui.Panel {
	...
	if recent := moduTUIWorkflowCockpitRecentRunLines(states); len(recent) > 0 {
		lines = append(recent, append([]string{""}, lines...)...)
	}
	if preview := moduTUIWorkflowCockpitPreviewLines(states); len(preview) > 0 {
...
func moduTUIWorkflowCockpitRecentRunLines(states map[string]any) []string {
...
func moduTUIWorkflowCockpitPreviewLines(states map[string]any) []string {
	state, ok := moduTUIWorkflowState(states)
	if !ok {
		return nil
	}
...
func moduTUIWorkflowNavigationShortcuts(runID string, views ...string) []modutui.PanelShortcut {
	...
	for _, view := range views {
		...
		case "result":
			shortcuts = append(shortcuts, modutui.PanelShortcut{Key: "o", Label: "Result", Command: moduTUIWorkflowPanelResultPrefix + runID})
		case "script":
			shortcuts = append(shortcuts, modutui.PanelShortcut{Key: "s", Label: "Script", Command: moduTUIWorkflowPanelScriptPrefix + runID})
		}
	}
	return shortcuts
}
