...
输入 `/tool-output <call-id>` 会从当前 session 的工具结果 artifact 读取完整本地输出；如果该工具结果没有 artifact，则回退显示模型看到的 preview。
...
飞书运行诊断会写入 `~/.modu/channels/feishu/debug.log`，不进入 TUI。
...
运行状态显示在聊天输入框上方：当前轮次只显示简短状态；轮次结束保留最近一轮完成耗时。输入框下方保留简短上下文用量、模型和工作区路径。任务进行中时，输入框上方还会显示本轮收到的活跃 todo 卡片；空闲、已完成、空列表、全完成或仅有上一轮遗留 todo 时隐藏。
当自动或手动 context compaction 完成时，TUI 会在 transcript 中持久插入 `------------- context compact ------------------` 分隔线；恢复 session 时也会按已保存的 compaction entry 回放该分隔线。
