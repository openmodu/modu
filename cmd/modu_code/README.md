...
`modu_code` 是一个跑在终端里的 AI 编程助手：它在当前工作目录里读写、搜索、执行代码，默认用 Bubble Tea inline TUI 渲染——完成的对话进入终端 scrollback，可以像 Claude Code / Codex 一样直接选中复制。
...
一句话选型：如果你想要一个可以接任意 OpenAI 兼容模型（本地 LM Studio / Ollama，或 DeepSeek / OpenAI / Anthropic）、能在 SSH 和手机终端上用、并且把工作流和定时任务内建进来的终端 coding agent，就是它。
