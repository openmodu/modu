...
Mailbox 是多 Agent 协作的状态机，负责注册、收件箱、任务、项目、能力队列、对抗式验证、流水线和对话记录。同一进程内直接使用 `Hub`；跨进程时使用 Redis 协议服务器。

[English](mailbox.md) | [中文](mailbox.zh-CN.md)

Mailbox 不运行 LLM，也不保证消息一定持久化。默认存储只在内存中保存状态，收件箱读取不会阻塞，事件订阅者在消费过慢时可能丢事件。进程边界和失败行为见 [Mailbox Agent 系统架构](../architecture/mailbox-agent-system.md)。
...
Agent 分布在不同进程或机器时，使用 `client.MailboxClient`。客户端通过 Redis 线协议发送自定义命令；Mailbox Server 本身不是 Redis 数据库。
...
| 对抗式验证 | `PublishValidatedTask`、`SubmitForValidation`、`SubmitValidation` | 独立 validator 复核结果，失败时可以重新入队 |
| 流水线 | `PublishPipeline`、`ClaimTask`、`CompleteTask` | 按能力匹配顺序执行，上一步结果可进入下一步描述 |
...
steps := []mailbox.PipelineStep{
    {DescriptionTemplate: "调研主题", RequiredCaps: []string{"research"}},
    {DescriptionTemplate: "根据以下材料写作：{{.PrevResult}}", RequiredCaps: []string{"writing"}},
}
...
流水线至少包含两个步骤。发布后，第一个步骤作为 Swarm 任务入队；完成一步时，Hub 会渲染下一步描述并按能力要求入队。`Store` 不保存 Pipeline 对象，因此 Hub 重启后不能继续原流水线的编排状态。
...
- **任务恢复**：Agent 被驱逐时，其持有的 Swarm 任务会重新入队，直到达到 `maxTaskRecoveries`；普通手动分配任务不会被自动改成 Swarm 任务。
- **流水线持久化**：`Store` 不保存 Pipeline 对象，Hub 重启后不能继续原流水线的编排状态。
- **并发安全**：Hub 方法会同步共享状态，但调用方仍需自己处理重试和幂等。
