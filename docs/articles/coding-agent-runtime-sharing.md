# modu code 运行时设计：从命令入口到可恢复会话

modu code 不是带工具的聊天程序，而是一套面向代码修改的 Agent 运行时。本文沿一次请求的执行路径，解释命令宿主、编码会话和 ReAct 内核如何分工。
...
[`docs/architecture/coding-agent.md`](../architecture/coding-agent.md) 将它定义为 L0-L5 分层。分享时不用逐层背，但要讲出一个核心原则：
...
这个边界决定了扩展成本。若 loop 直接承载权限、会话和产品状态，每加一种宿主能力都要修改主循环；modu code 的 loop 只暴露 hook 和 event，上层通过组合接入这些能力。
...
| 5 分钟 | tools + approval + context + session | 解释失败边界和恢复路径 |
| 3 分钟 | extensions + worktree + runtime state | 解释扩展入口和运行状态 |
| 3 分钟 | 完整链路和总结 | 帮听众收束 |
