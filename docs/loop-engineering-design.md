...
- **triage-fixes contract check(2026-07-04)**:`examples/loops/morning-triage/verify-triage-fixes-contract.sh` 同时检查 example 和本仓库 `.coding_agent/workflows/triage-fixes.js`:worktree 隔离、adversarial reviewer、结构化 verdict、PASS 才 `gh pr create --draft`、REJECT 写 inbox、批量上限 3,并要求 PASS 路径先 `git push -u origin fix/<slug>`,再用 `gh pr create --draft --base feat/loop --head fix/<slug>` 非交互开 PR,最后把 draft PR URL 持久写回 `state/triage.md` 的同一条 `pr-open` 行;同时检查 cron completion notification 仍会把新 inbox 条目和 GitHub PR 链接带给人审。它是 reviewer/draft PR 路径的静态合约验收,不替代真实 actionable finding 触发后的实跑。新增 `examples/loops/morning-triage/verify-triage-fixes-live.sh` 作为真实路径验收:要求本仓库 `workflow:triage-fixes` 证据里有 reviewer、`gh pr create --draft`、`state/triage.md` 更新路径,并且 verifier 只接受 `gh pr create` 实际输出的 PR URL 与当前 `state/triage.md` 的 `pr-open` URL 相交,再用 `gh pr view` 确认该 PR 仍是 open draft。这样不会把 Load 阶段读到的旧 PR URL 当成本次 Deliver 创建的 PR。`examples/loops/morning-triage/verify-triage-fixes-live-fixtures.sh` 用本地正例和缺 `pr-open`、缺 state PR URL、created PR 未写回 state、缺 reviewer、缺 draft 命令、PR 仓库不匹配、错误 workflow session、错误 cwd 负例测试 live verifier 自身,但不替代真实 draft PR。
...
- **triage-fixes Deliver finding 的 Fix/Review/PR(2026-07-04)**:把 Deliver 非交互缺口登记为 `state/triage.md` 的真实 open finding 后,再次跑 foreground `triage-fixes`;workflow run `20260704T032025.510037000Z` 的 Load 只读 state,Fix agent 产出分支 `fix/deliver-phase-prompt`,Review agent 执行合约检查并给出 `PASS`,Deliver agent 随后执行 `git push -u origin fix/deliver-phase-prompt`,再执行 `gh pr create --draft --base feat/loop --head fix/deliver-phase-prompt`,GitHub 返回 draft PR https://github.com/openmodu/modu/pull/77,并把同一个 URL 写回 `state/triage.md` 后提交 `d278fb84 triage: update finding to pr-open with draft PR #77`。这补齐了同一 workflow session 内 reviewer → draft PR → state persistence 的 strict live 证据;`verify-triage-fixes-live.sh` 现在明确通过 PR #77。
- **A 股 watchlist loop 改造(2026-07-04)**:`examples/workflows/a-stock-daily-report.js`、`examples/loops/morning-triage/tasks.yaml` 和真实 `~/.modu/cron/task.yaml` 的 `morning-market-daily` 已要求先读 `state/watchlist.md`,输出上一轮复盘,结束前完整写回 `state/watchlist.md`(题材/标的/明日验证点/移出项),并在写完后读回确认标题、最新日期、题材表、标的表和已移出观察表存在;workflow 日期锚点改为 Asia/Shanghai,避免 UTC 日期污染状态文件。workflow 数据采集已从 4 路并发改为串行,并要求优先 bash 直连腾讯/同花顺/东财公开 API,web_search/web_fetch 只作补充;东财请求必须串行限流,避免违反 `a-stock-data` 的防封约束。周末运行只读取上一轮 watchlist 并直接返回,不写 `state/watchlist.md`,避免把非交易日空数据写成有效观察。新增 `examples/workflows/verify-a-stock-workflow-contract.sh` 检查 workflow 合约,`examples/workflows/verify-a-stock-watchlist.sh` 检查实跑后的写回结构(标题、日期、复盘、题材/标的/移出表头和 Markdown 分隔行、复盘结论词),`examples/workflows/verify-a-stock-loop.sh` 再把非周末 watchlist 日期绑定到一条 scheduler-triggered、timezone=Asia/Shanghai、has_goal=true、goal 文本提到 `state/watchlist.md`、goal_verifier=true、goal_status=complete 的 `morning-market-daily` cron log 和完整 `cron:morning-market-daily` session,并要求该 session 同时留下同一 watchlist 日期、`state/watchlist.md` 读写、直连行情源(`qt.gtimg.cn`/东财/同花顺)、watchlist 更新证据和写后读回确认。`examples/workflows/verify-a-stock-loop-fixtures.sh` 用最小正例和缺数据源/缺写回/缺写后读回/日期不一致负例测试 live verifier 自身,但不替代真实交易日 scheduler 证据。当前没有创建真实 `state/watchlist.md`,因为 2026-07-04 00:00 是周六非早盘,不把无效行情实盘伪装成验收。
- **loop readiness preflight(2026-07-04)**:`examples/loops/morning-triage/verify-loop-readiness.sh` 检查本仓库 `.coding_agent/` 已安装 morning-triage skill、triage-fixes workflow、adversarial reviewer,并检查真实 `~/.modu/cron/config.yaml`/`task.yaml` 指向本仓库、`morning-triage` 与 `morning-market-daily` 都声明 `goal:`、`timezone: Asia/Shanghai`、状态文件路径和 cap 存在。它还要求 live A 股 prompt 固定包含 `# A股观察清单`、题材/标的/已移出观察表头和写后读回检查,避免未来交易日跑出不可验证的 state 形状。它证明安装面就绪,但仍不替代两工作日 scheduler log 或交易日 `state/watchlist.md` 实跑。
- **next-run window preflight(2026-07-04)**:`examples/loops/morning-triage/verify-next-run-windows.sh` 通过真实 `pkg/cron/scheduler.Next` 计算 live config 的下一次触发,并要求两个目标 loop enabled、带 `goal:`、timezone 为 `Asia/Shanghai`,且下一次触发分别落在上海工作日 06:00(`morning-triage`)和 10:20(`morning-market-daily`)。`verify-next-run-windows-fixtures.sh` 用离线正例和缺 goal、错时间、错时区、周末触发负例测试 verifier 自身,并已纳入 final gate。它证明等待的自然 tick 是目标窗口,不替代 tick 发生后的 run evidence。
- **scheduler armed preflight(2026-07-04)**:已用 detached tmux 会话 `modu-loop-cron` 启动本仓库 `go run ./cmd/modu_code`,daemon log 显示 `loaded 2 task(s)` 与 `cron scheduler started`。新增 `examples/loops/morning-triage/verify-scheduler-running.sh` 检查交互式 `modu_code` 进程、daemon lifecycle 和可选 tmux session;指定 tmux 时还要求 pane 当前目录是本仓库并且进程树里确实有 `modu_code`。该 preflight 还会比较 `modu_code` 进程启动时间和 `cmd/`、`pkg/` 下最新 `.go` 源文件 mtime,避免 Go harness 改完后忘记重启 scheduler,导致未来自然 cron 跑旧代码;同时比较当前 daemon lifecycle 的最新 task load/reload 时间与 cron config/task file mtime,避免 live task prompt 改完但 fsnotify 未 reload 时误以为自然 tick 会跑新任务。`verify-live-loop.sh` 默认按 `modu-loop-cron` 检查,用于证明未来自然 cron 有机会触发;它不替代最终 run 证据。
- **live-loop final gate(2026-07-04)**:`examples/loops/morning-triage/verify-live-loop.sh` 聚合 readiness、embedded scheduler running preflight、next-run window fixture、high-frequency canary 证据及其 fixture、triage-fixes empty-queue smoke、triage-fixes + notification contract、triage-fixes live verifier fixture、真实 triage-fixes reviewer/draft PR 证据、A 股 workflow contract、A 股 live verifier fixture、morning-triage two-day verifier fixture、严格两工作日自然 cron 证据和 A 股 scheduler-triggered/verifier-enabled/goal-complete watchlist loop 证据。默认严格失败才是现在的真实状态;`ALLOW_BOOTSTRAP=1` 仅用于查看 day-1 手动 + day-2 runner 的过渡证据。
