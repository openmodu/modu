...
ACP 的 `session/update` 通知有多种 `sessionUpdate` 子类型，每种映射到一个 modu `Event`：

| ACP `sessionUpdate` | modu `Event` | 备注 |
|---|---|---|
...
**纯函数**，不持有状态；输入 `*jsonrpc.Message`，输出 `[]Event`。方便单测。
...
### 5.4 为什么 bridge 是纯函数？
- 单测最好写（给 ACP notification fixture → 断言产出的 `Event`）
...
| bridge | 单测：加载 fixture notifications（JSON 文件），断言输出的 Event 序列 |
