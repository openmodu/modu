...
`modu_code` 默认注册 `extension/workflow`，提供 Lua 脚本驱动的 `workflow` tool，用来对齐 `pi-dynamic-workflows` 的动态多 agent 编排能力。当前实现支持 `meta` 声明、运行时 `phase`、`log`、`agent`、`parallel`、`pipeline`、`json.encode` / `json.decode`、预算视图、tool update 进度，以及基于 `ExtensionAPI.ForkSession` 的隔离子 agent 执行。`agent` / `parallel` task 可传 `label`、`phase`、`model`、`cwd`、`isolation:"worktree"`、`tools`、`disallowed_tools`、`permission_mode`、`max_turns`、`thinking`、`skills` 和 `memory_scope`，这些字段会映射到 forked session。`pipeline` 支持每个 item 顺序经过 stage，并按 `concurrency` 调度 item；由于同一个 Lua VM 不能并发执行字节码，stage 函数访问会串行保护，真正的多 agent fan-out 应优先用 `parallel`。
