...
- Start with ` + "`" + `meta({ name, description, phases, exampleArgs })` + "`" + `. Use a kebab-case name, a one-line description, unique phase titles, and a JSON-safe exampleArgs object whose keys match the input read from ` + "`" + `args` + "`" + `. When phases are declared, every ` + "`" + `phase(title)` + "`" + ` call and ` + "`" + `opts.phase` + "`" + ` value must use one of those titles. Then ` + "`" + `await` + "`" + ` at least one ` + "`" + `agent()` + "`" + ` and end the script with ` + "`" + `return <result>` + "`" + `.
...
- Minimal shape:
  ` + "`" + `meta({ name: "review-files", description: "Review changed files.", phases: [{ title: "Review" }], exampleArgs: { files: ["pkg/a.go"] } });` + "`" + `
  ` + "`" + `phase("Review");` + "`" + `
...
func hasToolNamed(tools []types.Tool, name string) bool {
