// Package bgtask is the background async-task service: it tracks spawned
// subagent runs, persists their status under the agent runtime dir, and
// supports cancellation. It implements taskoutput.Store.
package bgtask
...
func (m *Manager) SetStorePath(path string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.path = path
...
func (m *Manager) Create(kind, summary string) string {
	return m.CreateWithMetadata(kind, summary, "", "", "", "")
}

func (m *Manager) CreateWithMetadata(kind, summary, agentName, task, parentID, outputFile string) string {
...
func (m *Manager) CreateWithMetadataInDir(kind, summary, agentName, task, parentID, outputFile, runDirParent string) string {
	m.mu.Lock()
	m.nextID++
...
func (m *Manager) Interrupt(id, reason string) (taskoutput.Task, bool) {
	m.mu.Lock()
	task, ok := m.tasks[id]
	if !ok {
		m.mu.Unlock()
...
func (m *Manager) Get(id string) (taskoutput.Task, bool) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	task, ok := m.tasks[id]
	return task, ok
...
func (m *Manager) List() []taskoutput.Task {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *Manager) ensureRunPathsLocked() {
	for id, task := range m.tasks {
		m.tasks[id] = m.withRunPathsLocked(task)
	}
}

func (m *Manager) withRunPathsLocked(task taskoutput.Task) taskoutput.Task {
...
func (m *Manager) persistRunStatusLocked(task taskoutput.Task) {
	if strings.TrimSpace(task.StatusFile) == "" {
		return
	}
...
func (m *Manager) loadRunStatusesLocked() {
	if strings.TrimSpace(m.runRoot) == "" {
		return
	}
