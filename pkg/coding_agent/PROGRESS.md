...
- Added focused tests for:
  ...
  Upgraded `web_fetch` extraction to return Markdown from Trafilatura-cleaned article HTML, preserving links through `html-to-markdown`, and added `cmd/web_fetch` as a standalone CLI with Markdown, raw, JSON, max-byte, timeout, and file-output modes. Focused tests cover local HTML extraction and CLI JSON/Markdown output.
