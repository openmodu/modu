...
// New creates a provider that speaks the OpenAI chat completions protocol.
// id identifies the provider (e.g. "openai", "lmstudio").
func New(id string, opts ...Option) providers.Provider {
	var cfg Config
	for _, o := range opts {
		o(&cfg)
	}
...
func (p *openAIProvider) Stream(ctx context.Context, req *providers.ChatRequest) (types.EventStream, error) {
...
func (p *openAIProvider) buildBody(req *providers.ChatRequest, stream bool) ([]byte, error) {
	if req.Model == "" {
