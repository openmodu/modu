...
// SkillsLoader loads skill definitions from the filesystem.
// Priority: workspace > global > builtin.
type SkillsLoader struct {
	workspace       string
	workspaceSkills string // workspace skills (project-level)
	globalSkills    string // global skills (~/.modu/skills)
	builtinSkills   string // builtin skills
}

// NewSkillsLoader creates a new SkillsLoader.
//   - workspace: root workspace directory (project-level skills live at {workspace}/skills/)
//   - globalSkills: path to user-global skills directory (e.g. ~/.modu/skills)
//   - builtinSkills: path to the built-in skill definitions shipped with the binary
...
// ListSkills returns metadata for all available skills, in priority order
// (workspace > global > builtin). Duplicate names are skipped.
...
// LoadSkill reads the SKILL.md for the named skill and returns its content
// (frontmatter stripped). Returns ("", false) if not found.
...
// LoadSkillsForContext loads multiple skills and joins them into a formatted string
...
func (sl *SkillsLoader) getSkillMetadata(skillPath string) *SkillMetadata {
	...
	if err != nil {
		slog.Warn("failed to read skill metadata", "skill_path", skillPath, "error", err)
		return nil
	}
