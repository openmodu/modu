...
新建/查询/删除任务没有 CLI 命令——直接跟 `modu_code` 说话就行。builtin `cron` 扩展会注册 `/cron` slash command：

- `/cron add <自然语言需求>`：进入 agent 自然语言交互，由 agent 收集信息并调用 cron 工具新增任务
- `/cron list`：不走 agent，直接读取 `tasks.yaml` 并格式化展示 UUID/name/cron/enabled/prompt
...
任务身份分两层：`uuid` 是唯一身份，用于调度去重、删除、更新、日志目录和通知里的 `task_id`；`name` 是可读名称，来自旧格式的 `id`，允许重复。旧 `id:` 配置加载后会迁移为 `name:` 并自动补 `uuid:`，下次工具保存时不再写 `id:`。

## Agent 工具集

调度器跑任务时,任务自己的 agent 也会获得这 4 个管理 cron 任务表的工具（用于自我调度类的 prompt,例如"看看有没有该建的任务"）：

| 工具 | 作用 |
|------|------|
| `cron_add` | 加新任务（参数：name / cron / prompt / goal / timezone / enabled / on_overlap / channels；uuid 自动生成）|
| `cron_list` | 列出当前所有任务和已配置通知 channel，任务行包含 UUID 和 name |
| `cron_remove` | 按 UUID 删任务 |
| `cron_update` | 按 UUID 修改任务（可改 name / cron / prompt / goal / timezone / enabled / on_overlap / channels）|
...
> `cron_add` / `cron_remove` / `cron_update` 会用 `yaml.Marshal` 重写 `tasks.yaml`，**用户在 YAML 里写的任务注释会丢失**。
...
- **精简运行日志**——`~/.modu/cron/logs/<task_uuid>/<timestamp>.log`,NDJSON,见下面「任务日志」。`run_start` 同时记录 `task_id`(UUID) 和 `task_name`
- **完整 session 记录**——像交互式会话一样存进 `~/.modu/sessions/`,session 名是 `cron:<task_name>:<uuid-prefix>`,所以能在 session 列表/`--resume` 里按任务名和短 UUID 找到某次 cron 运行的完整对话(工具调用参数、thinking、原始事件都在,不是精简版)
...
```
~/.modu/cron/logs/<task_uuid>/<local-RFC3339-timestamp-with-ns>.log
```

```
ls ~/.modu/cron/logs/<task_uuid>/                 # 列出该任务所有 run,文件名即时间戳
tail -1 ~/.modu/cron/logs/<task_uuid>/*.log        # 看最新一次 run 的最后一行(通常是 run_end)
jq . ~/.modu/cron/logs/<task_uuid>/<file>.log      # 格式化看某一次完整事件流
```
...
| `type` | 字段 | 含义 |
|--------|------|------|
| `run_start` | `task_id` / `task_name` / `prompt` / `trigger` / `timezone` / `has_goal` / `goal` / `goal_verifier` / `started_at` | 本次 tick 起始；`task_id` 是 UUID,`task_name` 是可读名称；`trigger=scheduler` 是自然调度器触发,`manual` 是手动 harness/bootstrap；配置了 `timezone:` 的任务会记录该 IANA timezone；`has_goal=true` 表示本 tick 由任务级 `goal:` 驱动；声明 `goal:` 的任务会记录去空白后的 `goal` 文本和 `goal_verifier=true/false`,用于证明 completion 是否有 maker-checker verifier 门 |
...
被丢弃的事件：`agent_start`/`turn_start`/`message_start`/`message_update` 等 envelope 与 per-token 增量、`interrupt`、`thinking` 块、只含 tool call 没真文本的 assistant turn、`toolResult` role 的 message（信息已在 `tool_result` 里）。典型 100+ 行原始事件压到 < 10 行。这份精简版对应完整版就是上一节说的 `~/.modu/sessions/` 里那份 `cron:<task_name>:<uuid-prefix>` session。
...
老的单文件 `config.yaml` 内联 `tasks:` 仍可读；旧任务里的 `id:` 会作为 `name:` 读取并自动补 UUID。一旦通过 `cron_add` / `cron_remove` / `cron_update` 管理任务，新任务表会写入 `tasks_file`。

### 完成通知 `channels`

任务可以配置 `channel: <name>` 或 `channels: [<name>, ...]`。每次运行结束后（成功或失败都会触发），调度器会按任务引用的 channel 发送完成通知，内容包含任务 UUID、任务 name、状态、耗时、日志路径，以及日志中最后一段 assistant 文本。
...
├── crontools/              # cron_add / cron_list / cron_remove / cron_update agent 工具(flock)
