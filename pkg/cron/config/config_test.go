...
func TestTaskEffectiveTimeout(t *testing.T) {
	cases := []struct {
		...
		{"45m", 45 * time.Minute},
		{" 2h ", 2 * time.Hour},
		{"bogus", DefaultRunTimeout},
		{"-5m", DefaultRunTimeout},
	}
	for _, c := range cases {
		if got := (Task{Timeout: c.in}).EffectiveTimeout(); got != c.want {
			t.Errorf("Task{Timeout=%q}.EffectiveTimeout() = %v; want %v", c.in, got, c.want)
		}
	}
}
...
func TestTaskValidateCaps(t *testing.T) {
	...
	if err := (Task{ID: "a"}).ValidateCaps(); err != nil {
		t.Fatalf("zero-value caps rejected: %v", err)
	}
...
func TestCapFieldsRoundTrip(t *testing.T) {
	...
	if out.DailyBudgetTokens != 3_000_000 {
