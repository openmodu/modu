...
// highlighted; pending items are dimmed. Returns "" when there are no todos so
// the panel takes no vertical space until the agent populates the list.
func (b *bubbleTUI) renderTodoPanel() string {
	...
	var lines []string
	lines = append(lines, "  "+uiDimText.Render("Todos"))
